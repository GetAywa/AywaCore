...
*Setup instructions for a Gitian build of Aywa Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Aywa
...
- [Building Aywa Core](#building-aywa-core)
...
Clone the git repositories for Aywa Core and Gitian.
...
in the Aywa Core repository under 'Fetch and build inputs' to install sources which require
...
To build Aywa Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Aywa Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/aywa/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/aywapay/aywa
...
and the aywa git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../aywa/contrib/gitian-descriptors/*|sort|uniq )
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/aywa-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/aywa-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/aywa-osx-build.assert
```

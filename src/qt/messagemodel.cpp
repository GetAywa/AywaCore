...
const QString MessageModel::Channel = "Channel";
const QString MessageModel::ChannelSent = "ChannelSent";
...
public:
    ...
    void refreshMessageTable()
    {
        ...
        //try reset model
        //parent->walletModel->requestUnlock();
            ...
            while (dbSmsg.NextSmesg(it, sPrefix, chKey, smsgStored))
            {
                ...
                if (SecureMsgDecrypt(false, smsgStored.sAddrTo, &smsgStored.vchMessage[0], &smsgStored.vchMessage[SMSG_HDR_LEN], nPayload, msg) == 0)
                {
                    ...
                    addMessageEntry(MessageTableEntry(vchKey,
                                        ...
                                        true);
//                    if (smsgStored.status == SMSG_MASK_UNREAD) nUnreadMessagesNumber++;
                }
            };
                ...
                if (SecureMsgDecrypt(false, smsgStored.sAddrOutbox, &smsgStored.vchMessage[0], &smsgStored.vchMessage[SMSG_HDR_LEN], nPayload, msg) == 0)
                {
                    ...
                    addMessageEntry(MessageTableEntry(vchKey,
                                                      ...
                                                      GetIsChannelSubscribed(smsgStored.sAddrTo)|GetIsChannelSubscribed(msg.sFromAddress) ?
                                                          MessageTableEntry::ChannelSent : MessageTableEntry::Sent,
//                                                      QString (smsgStored.status == SMSG_MASK_UNREAD ? "1" : "0"),
                                                      smsgStored.status == SMSG_MASK_UNREAD ? true : false,
                                                      label,
                                                      QString::fromStdString(smsgStored.sAddrTo),
                                                      QString::fromStdString(msg.sFromAddress),
    ...
    void walletUnlocked()
    {
        ...
        if (parent->proxyModel)
        {
            parent->proxyModelContacts->setFilterRole(false);
            parent->proxyModelContacts->setFilterFixedString("");
            parent->resetFilter();
            parent->proxyModelContacts->setFilterRole(MessageModel::Ambiguous);
            parent->proxyModelContacts->setFilterFixedString("true");
        }
        //invalidateFilter()
        //refreshMessageTable();
    }
...
private:
    ...
    void handleMessageEntry(const MessageTableEntry & message, const bool append)
    {
        ...
        UniValue mVal;
        mVal.read(message.message.toStdString());
        if(mVal.isNull())
        {
            addMessageEntry(message, append);
            return;
        ...
        UniValue mObj(mVal.get_obj());
        UniValue mvType = find_value(mObj, "type");
    ...
    void addMessageEntry(const MessageTableEntry & message, const bool & append)
    {
        ...
        {
            int index = qLowerBound(cachedMessageTable.begin(), cachedMessageTable.end(),
                                    message.received_datetime,
                                    MessageTableEntryLessThan()) - cachedMessageTable.begin();
...
MessageModel::StatusCode MessageModel::sendMessages(const QList<SendMessagesRecipient> &recipients, const QString &addressFrom)
{
    ...
    Q_FOREACH(const SendMessagesRecipient &rcp, recipients)
    {
...
QVariant MessageModel::data(const QModelIndex &index, int role) const
{
    ...
    case Qt::DisplayRole:
        switch(index.column())
        {
            case Label:	           return (rec->label.isEmpty() ?
                                               (rec->type == MessageTableEntry::Sent ?
                                                    rec->to_address : rec->from_address)  : rec->label);//TODO: Channel name better?
        ...
        case Type:
            ...
            case MessageTableEntry::Received: return Received;
                //added
            case MessageTableEntry::ChannelSent:     return ChannelSent;
            case MessageTableEntry::Channel: return Channel;
            default: break;
            }
    ...
    case FilterAddressRole: return (rec->type != MessageTableEntry::Received ?
                    rec->to_address : rec->from_address);


    case LabelRole:         return rec->label;
    case MessageRole:       return rec->message;
    case UnreadFlagRole:    return rec->unread_flag;
    ...
    case Ambiguous:
        ...
        for (it = 0; it<ambiguous.length(); it++) {
            if(ambiguous[it] == (rec->type == 2 ?
                                 rec->from_address : rec->to_address)) //commented 20180901
                return false;
        }

        QString address = rec->type == 2 ? rec->from_address : rec->to_address;//added 20180921
...
void MessageModel::UpdateAddressBook (std::string strAddress, std::string strLabel)
{
    // Add addresses / update labels that we've sent to to the address book
    CTxDestination dest = CBitcoinAddress(strAddress).Get();
    {

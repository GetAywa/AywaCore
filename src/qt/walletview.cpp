    ...
    platformStyle(platformStyle)
{
    ...
    //Pass through messages from masternodeListPage(Proposal) TODO
...
void WalletView::setWalletModel(WalletModel *walletModel)
{
    ...
    messagePage->setModel(walletModel->getMessageModel());
    if (walletModel)
    {
        ...
        connect(walletModel->getMessageModel(), SIGNAL(rowsInserted(QModelIndex, int, int)),
                    this, SLOT(incomingMessageNotify(QModelIndex, int, int)));
    }
}

void WalletView::incomingMessageNotify(const QModelIndex& parent, int start, int /*end*/)
{
    // Prevent balloon-spam when initial block download is in progress
    if (!walletModel || !clientModel || clientModel->inInitialBlockDownload())
        return;
    // On new transaction, make an info balloon
    MessageModel *mm = walletModel->getMessageModel();

    //MessageTableEntry::Type qiMessageType = mm->index(start, MessageModel::ReceivedDateTime, parent);
        if (mm->index(start, MessageModel::TypeInt, parent).data().toInt() >= MessageTableEntry::Received)
    //if (qiMessageType == MessageTableEntry::Received)
        {
            ...
            QString from_label = mm->index(start, MessageModel::Label, parent).data().toString();
            //QString to_address = mm->index(start, MessageModel::ToAddress, parent).data().toString();
            QString qmessage = mm->index(start, MessageModel::Message, parent).data().toString();
            QTextDocument html;
            html.setHtml(qmessage);
    ...
    msg +=tr("Date: %1\n").arg(sent_datetime);// +
                      ...
                      //tr("Type: %1\n").arg(type);
         if (!from_label.isEmpty()) msg += tr("From: %1\n").arg(from_label);
            else if (!from_address.isEmpty()) msg += tr("From: %1\n").arg(from_address);//TODO:Change for name, if possible.
        if (!messageText.isEmpty())
        msg += tr("%1\n").arg(messageText);
        message(tr("New message recieved"),
...
void WalletView::gotoMessagePage()
{
   ...
   // Unlock wallet when requested by wallet model
    ...
    if (walletModel->getEncryptionStatus() == WalletModel::Locked || walletModel->getEncryptionStatus() == WalletModel::UnlockedForMixingOnly) {
        ...
        QMessageBox msgBox;
        msgBox.setText("Aywa-QT");
        msgBox.setInformativeText("Wallet is password locked. To use messaging unlock you Aywa wallet.");

...
UniValue masternode(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp  ||
        (
         ...
         strCommand != "connect" && strCommand != "status" && strCommand !="cost"))
            throw std::runtime_error(
                "masternode \"command\"...\n"
                ...
                "  cost         - Print current masternode collateral tx required\n"
#endif // ENABLE_WALLET
    ...
    if (strCommand == "cost")
    {
        return GetMNCollateralAmount(chainActive.Tip()->GetBlockTime());
    }
...
UniValue masternodelist(const UniValue& params, bool fHelp)
{
                ...
                strMode != "rank" && strMode != "status"))
    {
        throw std::runtime_error(
                ...
                "  payee          - Print Aywa address associated with a masternode (can be additionally filtered,\n"
                "                   partial match)\n"

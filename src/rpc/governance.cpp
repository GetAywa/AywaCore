...
UniValue gobject(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in aywa.conf\n"
    ...
    */
    ...
    if(strCommand == "serialize")//convert string to HEX
    {
        if (params.size() != 2) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Correct usage is 'gobject serialize <data-str>'");
        }

        std::string strString = params[1].get_str();
        //std::string v = StringToHex(strString);
        string v = HexStr(strString);
        //std::string s(v.begin(), v.end());

        //UniValue u(UniValue::VOBJ);
        //u.read(s);

        return v.c_str();//u.write().c_str();
    }
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("aywa.conf", statusObj));

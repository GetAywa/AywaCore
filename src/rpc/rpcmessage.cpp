...
//                        objM.push_back(UniValue("key", address));
//                        objM.push_back(UniValue("publickey", sPublicKey));
//                        objM.push_back(UniValue("label", entry.second));
...
//UniValue smsgsendtochannel(const UniValue& params, bool fHelp)
//{
//    if (fHelp || params.size() != 3)
//        throw runtime_error(
//            "smsgsend <addrFrom> <addrTo> <message>\n"
...
//    std::string sError;
//    if (SecureMsgSend(addrFrom, addrTo, msg, sError, *g_connman) != 0)
//    {
//        result.push_back(Pair("result", "Send failed."));
//        result.push_back(Pair("error", sError));
//    } else
...
//                    //int it=0;
//                    std::string strPubKey, strPrivKev;
//                    BOOST_FOREACH(SecMsgChannel curChannel, smsgChannels) {
//                        if (curChannel.sAddress==smsgStored.sAddrTo) {
//                            strPubKey = curChannel.sPubKey;
//                            strPrivKev = curChannel.sPrivKey;
//                        }
//                    };
//                    if (SecureMsgDecryptChannel(false, smsgStored.sAddrTo, strPubKey,strPrivKev, &smsgStored.vchMessage[0], &smsgStored.vchMessage[SMSG_HDR_LEN], nPayload, msg) == 0)
//                    {
//                        //Object objM;
//                        UniValue objM(UniValue::VOBJ);
//                        objM.push_back(Pair("received", getTimeString(smsgStored.timeReceived, cbuf, sizeof(cbuf))));
...
//                snprintf(cbuf, sizeof(cbuf), "%" PRIszu, tokenSet.size());
                ...
                objM.push_back(Pair("time", getTimeString(it->first, cbuf, sizeof(cbuf))));//TODO!!!

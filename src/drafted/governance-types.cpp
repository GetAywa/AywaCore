...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "AywaNetwork") type = AywaNetwork;
    if(strType == "AywaNetworkVariable") type = AywaNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == AywaNetwork) s = "AywaNetwork";
    if(type == AywaNetworkVariable) s = "AywaNetworkVariable";

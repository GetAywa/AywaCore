...
const unsigned int SMSG_THREAD_DELAY    = 10;//30; TODO!  Change dynamically during dialog
...
extern std::vector<SecMsgChannel>       smsgChannels;
...
public:
    SecMsgChannel() {}
    SecMsgChannel(std::string sAddr, std::string sPKey, std::string sKey, bool receiveOn, bool receiveAnon, std::string sChannelName)
    {
        sAddress            = sAddr;
        sPubKey             = sPKey;
        sPrivKey            = sKey;
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion) {
        ...
        READWRITE(this->sPubKey);
        READWRITE(this->sPrivKey);
        READWRITE(this->fReceiveEnabled);
        READWRITE(this->fReceiveAnon);
        READWRITE(this->sName);
    }
};
...
public:
    SecMsgOptions()
    {
        ...
        fScanIncoming   = true;
        nSMSG_THREAD_DELAY = 10;
    }

    bool fNewAddressRecv;
    bool fNewAddressAnon;
    bool fScanIncoming;
    int nSMSG_THREAD_DELAY;
};
...
int SecureMsgSend(std::string &addressFrom, std::string &addressTo, std::string &message, std::string &sError);//, CConnman& connman

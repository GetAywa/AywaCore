...
CMasternode::CollateralStatus CMasternode::CheckCollateral(const COutPoint& outpoint, int& nHeightRet)
{
   ...
   //TODO test
    ...
    if(coin.out.nValue != GetMNCollateralAmount(nCTxBlockTime) * COIN)
...
bool CMasternode::IsInputAssociatedWithPubkey()
{
    ...
    if(GetTransaction(vin.prevout.hash, tx, Params().GetConsensus(), hash, true)) {
        ...
        {
            //TODO!!:OPTIMIZE CODE
//            int nTxTime = pwalletMain->GetWalletTx(out.GetHash())->nTimeSmart;
//            int nTxTime2 = pwalletMain->GetWalletTx(out.GetHash())->nTimeReceived;
//            int nTxTime = pwalletMain->GetWalletTx(out.GetHash())->GetTxTime();



            BlockMap::iterator it = mapBlockIndex.find(hash);
            CBlockIndex *pindex = it->second;
            int nCTxBlockTime  = pindex->GetBlockTime();



            if(out.nValue == GetMNCollateralAmount(nCTxBlockTime)*COIN && out.scriptPubKey == payee)
                return true;
        }
    }
...
bool CMasternodeBroadcast::Create(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& strErrorRet, CMasternodeBroadcast &mnbRet, bool fOffline)
{
    ...
    if(!sporkManager.IsSporkActive(SPORK_A017_ANY_MASTERNODE_PORT_ENABLED))
    {
...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
...

    {
        ...
        if (err == COLLATERAL_INVALID_AMOUNT) {
            LogPrint("masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have 1000 AYWA, masternode=%s\n", vin.prevout.ToStringShort());
            return false;
        }
    ...
    // should be at least not earlier than block when 1000 AYWA tx got nMasternodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex* pMNIndex = (*mi).second; // block for 1000 AYWA tx -> 1 confirmation

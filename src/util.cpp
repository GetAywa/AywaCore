...
const char * const BITCOIN_CONF_FILENAME = "aywa.conf";
const char * const BITCOIN_PID_FILENAME = "aywad.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "aywa" is a composite category enabling all Aywa-related debug output
            if(ptrCategory->count(string("aywa"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AywaCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AywaCore
    // Mac: ~/Library/Application Support/AywaCore
    // Unix: ~/.aywacore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AywaCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AywaCore";
#else
    // Unix
    return pathRet / ".aywacore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty aywa.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override aywa.conf

...
Notes:
    ...
    Address Whitelist
        ...
        Saved to smsg.conf
        Modify options using the smsglocalkeys rpc command or edit the smsg.conf file (with client closed)
...
void ThreadSecureMsg()
{
    ...
    //wait, if not loaded wallet data
    //we dont download messages if wallet not loaded yet
    //TODO Check it
    ...
    while (fSecMsgEnabled)
    {
        ...
        for (std::vector<std::pair<int64_t, NodeId> >::iterator it(vTimedOutLocks.begin()); it != vTimedOutLocks.end(); it++)
        {
            ...
            //connman.PushMessage(pnode,"smsgIgnore", vchData);
            // -- look through the nodes for the peer that locked this bucket
            //CConnman::ForEachNode()
            //connman.PushMessage("Hello");

            connman.ForEachNode(CConnman::AllNodes, [&nPeerId, &ignoreUntil, &connman](CNode* pnode) {
...
//            auto notifyfunc = [&nPeerId, &ignoreUntil, &connman](CNode* pnode) {
//                if (pnode->id = nPeerId) {
//                    LOCK2(pnode->cs_vSend, pnode->smsgData.cs_smsg_net);
//                    std::vector<uint8_t> vchData;
//                    vchData.resize(8);
...
//            connman.ForEachNodeThen(std::move(notifyfunc));
            {
                //LOCK(cs_vNodes);
...
int SecureMsgReadIni()
{
    ...
    fs::path fullpath = GetDataDir() / "smsg.conf";
    ...
    char cAddress[64], cPubKey[64], cKey[64];
    char cName[64];
...
//        if (strcmp(pName, "nSMSG_THREAD_DELAY") == 0)
//        {
//            smsgOptions.nSMSG_THREAD_DELAY = (strcmp(pValue, "10") == 0) ? true : false;
//        } else
        ...
        {
            //int rv = sscanf(pValue, "%64[^|]|%64[^|]|%64[^|]|%d|%d", cAddress, cPubKey, cKey, &addrRecv, &addrRecvAnon);
            int rv = sscanf(pValue, "%64[^|]|%64[^|]|%64[^|]|%d|%d|%64[^|]|", cAddress, cPubKey, cKey, &addrRecv, &addrRecvAnon, cName);
            if (rv == 6)//check count of returned values
            {
                smsgChannels.push_back(SecMsgChannel(std::string(cAddress), std::string(cPubKey), std::string(cKey), addrRecv, addrRecvAnon, std::string(cName)));
            } else
            {
                LogPrint("smessage", "Could not parse key line %s, rv %d.\n", pValue, rv);
            }
    ...
    LogPrint("smessage", "Loaded %u channels.\n", smsgChannels.size());
...
int SecureMsgWriteIni()
{
    ...
    if (fprintf(fp, "newAddressRecv=%s\n", smsgOptions.fNewAddressRecv ? "true" : "false") < 0
        ...
        //|| fprintf(fp, "nSMSG_THREAD_DELAY", smsgOptions.nSMSG_THREAD_DELAY ))
    {
    ...
    {
        errno = 0;
        if (fprintf(fp, "channel=%s|%s|%s|%d|%d|%s|\n", it->sAddress.c_str(), it->sPubKey.c_str(), it->sPrivKey.c_str(), it->fReceiveEnabled, it->fReceiveAnon, it->sName.c_str()) < 0)
        {
            LogPrint("smessage", "fprintf error: %s\n", strerror(errno));
            continue;
        };
    ...
    try {
        fs::path finalpath = GetDataDir() / "smsg.conf";
...
bool SecureMsgStart(bool fDontStart, bool fScanChain)
{
    ...
    if (SecureMsgReadIni() != 0)
        LogPrint("smessage", "Failed to read smsg.conf\n");
    ...
    if (smsgChannels.size()==0)
    {
        SetChannelSubscribtion("AYWAx1xFemXjBQQCJVxeeN5Q6mxw4crJQV", "sMpEJgT1QCVPoXCRX1qstqx2hd8vQkThVJtA8FacoZgp", "PKANREpXUJxUGAo5bzxFHP2egrUdh8QNH9SsfmeCU7y5TNyneNkt", "#general");
        SetChannelSubscribtion("AYWAx2xEM3xSKQ7wQGq7euNsTjWDRUYoaE", "f4z16VAb2zMaAraPaBfqr7pjwRaeSJMDfEmuZkYAAyc2", "PFuNAffeJwQKwgAtHFmrvXXvH6k4w7LKXSzJ3qzomFHdHDrzYqdT", "#support");
    }

    //and to SecureMsgEnable()
...
bool SecureMsgShutdown()
{
    ...
    if (SecureMsgWriteIni() != 0)
        LogPrint("smessage", "Failed to save smsg.conf\n");
...
bool SecureMsgEnable()
{
    ...
    */
    ...
    {
        //LOCK(cs_vNodes); //LOCK cs_vNodes included to ForEachNode

    } // cs_vNodes
    CConnman& connman = *g_connman;
    connman.ForEachNode(CConnman::AllNodes, [&connman](CNode* pnode) {
        connman.PushMessage(pnode,"smsgPing");
        connman.PushMessage(pnode,"smsgPong");
    });
...
//            //CConnman.PushMessage(pnode,"smsgDisabled");
...
//        };
    ...
    connman.ForEachNode(CConnman::AllNodes, [&connman](CNode* pnode) {
        ...
        if (pnode->smsgData.fEnabled){
            LOCK2(pnode->cs_vSend, pnode->smsgData.cs_smsg_net);
            //pnode->PushMessage("smsgDisabled");
            connman.PushMessage(pnode,"smsgDisabled");
            pnode->smsgData.fEnabled = false;
        }
    });
    if (SecureMsgWriteIni() != 0)
...
bool SecureMsgReceiveData(CNode* pfrom, std::string strCommand, CDataStream& vRecv, CConnman& connman)
{
    /*
    ...
    */
    ...
    if (strCommand == "smsgInv")
    {
        ...
        if (vchDataOut.size() > 4)
        {
            ...
            connman.PushMessage(pfrom, "smsgShow", vchDataOut);
        } else
        if (nLocked < 1) // Don't report buckets as matched if any are locked
        {
            ...
            connman.PushMessage(pfrom, "smsgMatch", vchDataOut);
            if (fDebugSmsg)
    ...
    if (strCommand == "smsgShow")
    {
        ...
        for (uint32_t i = 0; i < nBuckets; ++i, pIn += 8)
        {
            ...
            connman.PushMessage(pfrom, "smsgHave", vchDataOut);
        };
    ...
    if (strCommand == "smsgHave")
    {
        ...
        if (vchDataOut.size() > 8)
        {
            ...
            connman.PushMessage(pfrom, "smsgWant", vchDataOut);
        };
    } else
    if (strCommand == "smsgWant")
    {
        ...
        if (nBunch > 0)
        {
            ...
            connman.PushMessage(pfrom, "smsgMsg", vchBunch);
        };
    } else
    ...
    if (strCommand == "smsgPing")
    {
        ...
        connman.PushMessage(pfrom, "smsgPong");
...
bool SecureMsgSendData(CNode* pto, CConnman& connman)
{
    ...
    */
    ...
    if (pto->smsgData.lastSeen == 0)
    {
        ...
        connman.PushMessage(pto, "smsgPing");
...

    {
        ...
        if (nBuckets > 0) // no need to send keep alive pkts, coin messages already do that
        {
            ...
            if (vchData.size() > 4)
            {
                ...
                connman.PushMessage(pto, "smsgInv", vchData);
            };
        };
    } // cs_smsg
...
//static bool ScanBlock(CBlock& block, CBlockTreeDB& txdb, SecMsgDB& addrpkdb,
...

{
    ...
    // -- only scan inputs of standard txns

    BOOST_FOREACH(CTransaction& tx, block.vtx)
    {
    std::string sReason;
        /*/ - harvest public keys from coinstake txns (skipping)
        if (tx.IsCoinStake())
        {
        ...
        if (IsStandardTx(tx, sReason))
        {
            ...
            {
                const CScript *script = &tx.vin[i].scriptSig;//const added
...
bool SecureMsgScanBlock(CBlock& block)
{
    ...
    {
        LOCK(cs_smsgDB);
        //CBlockTreeDB txdb=NULL;//("r");
//        CWalletDB txdb();
        //CWalletDB txdb(pwalletMain->strWalletFile);
...
bool SecureMsgScanBlockChain()
{
    ...
    if (lockMain)
    {
//        CBlockIndex *pindexScan = ChainActive.Genesis();//pindexGenesisBlock;
...

        {
            ...
            if (dbInbox.Open("cw"))
            {
                ...
                } else
                {
                    ...
                    std::string strCmd = GetArg("-msginboxnotify", "");
                    if(!strCmd.empty()) {
                        //boost::replace_all(strCmd, "%s", txHash.GetHex()); //TODO - add time, sender, text
...
//int SecureMsgScanMessageChannel(uint8_t *pHeader, uint8_t *pPayload, uint32_t nPayload, bool reportToGui)
//{
//    /*
//    Check if message belongs to this node.
//    If so add to inbox db.
...
//    std::string addressTo, pubKey, privKey;
//    MessageData msg; // placeholder
//    bool fOwnMessage = false;

//    //TODO: improve&optimize code by megring with same modules. added for test purpose only
//        for (std::vector<SecMsgChannel>::iterator it = smsgChannels.begin(); it != smsgChannels.end(); ++it)
//        {
//            if (!it->fReceiveEnabled)
//                continue;
...
//            {
//                // -- have to do full decrypt to see address from
//                if (SecureMsgDecryptChannel(false, addressTo, pubKey, privKey, pHeader, pPayload, nPayload, msg) == 0)
//                {
//                    if (fDebugSmsg)
//                        LogPrint("smessage", "Decrypted channel message with %s.\n", addressTo.c_str());
...
//            //TODO: add channel flag to class SecMsgStored.
...
//            if (SecureMsgScanMessageChannel(&vchData[n], &vchData[n + SMSG_HDR_LEN], psmsg->nPayload, true) != 0)
//            {
//                // message recipient is not this node (or failed)
//                if (fDebugSmsg)
//                    LogPrint("smessage", "Message channel recipient is not this node (or failed).\n");
//            };
...
int SecureMsgStore(uint8_t *pHeader, uint8_t *pPayload, uint32_t nPayload, bool fUpdateBucket)
{
        ...
        || !pPayload)
    {
        ...
        error("null pointer to header or payload.");
        return 1;
    };
    ...
    } catch (const boost::filesystem::filesystem_error& ex)
    {
        ...
        error("Failed to create directory %s - %s.", pathSmsgDir.string().c_str(), ex.what());
        return 1;
    };
...
int SecureMsgValidate(uint8_t *pHeader, uint8_t *pPayload, uint32_t nPayload)
{
    ...
    */
    ...
    if (fDebugSmsg)
        LogPrint("smessage", "SecureMsgValidate() nonce %u.\n", nonce);
    ...
    } else
    {
        if (sha256Hash[31] == 0
            && sha256Hash[30] == 0
//            && (~(sha256Hash[29]) & ((1<<0) || (1<<1) || (1<<2)) ))//commented 20180921
                && (~(sha256Hash[29]) & ((1<<0) | (1<<1) | (1<<2)) ))//added 20180921
...
int SecureMsgSetHash(uint8_t *pHeader, uint8_t *pPayload, uint32_t nPayload)
{
    ...
    */
    ...
    for (;;)
    {
        ...
        */
        ...
        if (sha256Hash[31] == 0
            && sha256Hash[30] == 0
        //    && (~(sha256Hash[29]) & ((1<<0) || (1<<1) || (1<<2)) )) // commented 20180921
                && (~(sha256Hash[29]) & ((1<<0) | (1<<1) | (1<<2)) )) //added
        //    && sha256Hash[29] == 0) //uncommented 20180921
        {
            found = true;
            //if (fDebugSmsg)
            //    LogPrint("smessage", "Match %u\n", nonse);
    ...
    {
        if (fDebugSmsg)
            LogPrint("smessage", "SecureMsgSetHash() failed, took %d ms, nonce %u\n", GetTimeMillis() - nStart, nonce);
        return 1;
    };
    ...
    if (fDebugSmsg)
        LogPrint("smessage", "SecureMsgSetHash() took %d ms, nonce %u\n", GetTimeMillis() - nStart, nonce);
...
int SecureMsgEncrypt(SecureMessage &smsg, const std::string &addressFrom, const std::string &addressTo, const std::string &message)
{
    ...
    */
    ...
    if (message.size() > SMSG_MAX_MSG_BYTES)
    {
        ...
        error("%s: Message is too long, %u.", __func__, message.size());
        return 2;
    };
    ...
    } else
    {
        ...
        if (!coinAddrFrom.SetString(addressFrom))
        {
            ...
            error("%s: addressFrom is not valid.", __func__);
            return 3;
        };
        ...
        if (!coinAddrFrom.GetKeyID(ckidFrom))
        {
            ...
            error("%s: coinAddrFrom.GetKeyID failed: %s.", __func__, coinAddrFrom.ToString().c_str());
            return 4;
        };
    };
    ...
    if (!coinAddrDest.SetString(addressTo))
    {
        ...
        error("%s: addressTo is not valid.", __func__);
        return 4;
    };
    ...
    if (!coinAddrDest.GetKeyID(ckidDest))
    {
        ...
        error("%s: coinAddrDest.GetKeyID failed: %s.", __func__, coinAddrDest.ToString().c_str());
        return 4;
    };
    ...
    {
        //try to encrypt with channel keys
        std::string strPubKey, strPrivKey;
        if (!GetChannelKeys(addressTo, strPubKey, strPrivKey))
        {
            error("%s: Could not get public key for destination address.", __func__);
            return 5;
        }
        else
        {
            std::vector<uint8_t> vchPubKey;
            DecodeBase58(strPubKey, vchPubKey);
    ...
    CECKey ecKeyK;
    //CKey ecKeyK;
//    if (!ecKeyK.VerifyPubKey(cpkDestK))//cpkDestK.begin(), cpkDestK.size()))
    if (!ecKeyK.SetPubKey(cpkDestK.begin(), cpkDestK.size()))
    {
       ...
       // //return errorN(4, "%s: Could not set pubkey for K: %s.", __func__, HexStr(cpkDestK).c_str());
        error("%s: Could not set pubkey for K: %s.", __func__, HexStr(cpkDestK).c_str());
        return 4;
    };
...
//        //return errorN(6, "%s: ECDH_compute_key failed, lenP: %d.", __func__, lenP);
        error("%s: ECDH_compute_key failed, lenP: %d.", __func__);
        return 6;
    };
        ...
        || !cpkR.IsCompressed())
    {
        ...
        error("%s: Could not get public key for key R.", __func__);
        return 1;
    };
    ...
    //HashX11(&vchP[0], vchP.size(), (uint8_t*)&vchHashed[0]);
    ...
    if (lenMsg > 128)
    {
        ...
        try { vchCompressed.resize(worstCase); } catch (std::exception& e)
        {
            ...
            error("%s: vchCompressed.resize %u threw: %s.", __func__, worstCase, e.what());
            return 8;
        };
        ...
        if (lenComp < 1)
        {
            ...
            error("%s: Could not compress message data.", __func__);
            return 8;
        };
    ...
    if (fSendAnonymous)
    {
        try { vchPayload.resize(9 + lenMsgData); } catch (std::exception& e)
        {
            ...
            error("%s: vchPayload.resize %u threw: %s.", __func__, 9 + lenMsgData, e.what());
            return 8;
        };
    ...
    } else
    {
        try { vchPayload.resize(SMSG_PL_HDR_LEN + lenMsgData); } catch (std::exception& e)
        {
            ...
            error("%s: vchPayload.resize %u threw: %s.", __func__, SMSG_PL_HDR_LEN + lenMsgData, e.what(), __func__);
            return 8;
        };
        ...
        {
            //TODO: improve code below without a loop via iterator and find
            //int it=0;
            std::string strPrivKev;
            BOOST_FOREACH(SecMsgChannel curChannel, smsgChannels) {
                if (curChannel.sAddress==addressFrom) {
                    //strPubKey = curChannel.sPubKey;
                    strPrivKev = curChannel.sPrivKey;
                }
            };
            CBitcoinSecret vchSecret;
            if (!vchSecret.SetString(strPrivKev))
            {
                error("%s: Could not get private key for addressFrom.", __func__);
                return 7; }
            keyFrom = vchSecret.GetKey();
            if (!keyFrom.IsValid()){
                error("%s: Could not get private key for addressFrom.", __func__);
                return 7;
            }
    ...
    if (!crypter.Encrypt(&vchPayload[0], vchPayload.size(), vchCiphertext))
    {
        ...
        error("%s: crypter.Encrypt failed.", __func__);
        return 11;
    };
    ...
    try { smsg.pPayload = new uint8_t[vchCiphertext.size()]; } catch (std::exception& e)
    {
        ...
        error("%s: Could not allocate pPayload, exception: %s.", __func__, e.what());
        return 8;
    };
    ...
    if (!fHmacOk)
    {
        ...
        error("%s: Could not generate MAC.", __func__);
        return 10;
    };
...
int SecureMsgEncryptChannel(SecureMessage &smsg, const std::string &addressFrom, const std::string &addressTo, const std::string &message)
{
    /* Create a secure message
...
int SecureMsgSend(std::string &addressFrom, std::string &addressTo, std::string &message, std::string &sError)//TODO: remove connman from here (, CConnman& connman = *g_connman)
{
...
//int SecureMsgSendToChannel(std::string &addressFrom, std::string &addressTo, std::string &message, std::string &sError, CConnman& connman)
//{
//    /* Encrypt channel message, and place it on the network
//        Make a copy of the message to sender's first address and place in send queue db
//        proof of work thread will pick up messages from  send queue db
...
//    if ((rv = SecureMsgEncryptChannel(smsg, addressFrom, addressTo, message)) != 0)
//    {
//        LogPrint("smessage", "SecureMsgSend(), encrypt for recipient failed.\n");
...
int SecureMsgDecrypt(bool fTestOnly, std::string &address, uint8_t *pHeader, uint8_t *pPayload, uint32_t nPayload, MessageData &msg)
{
    ...
    */
    ...
    if (!pHeader
        || !pPayload)
    {
        error("%s: null pointer to header or payload.", __func__);
        return 1;
    };
    SecureMessage* psmsg = (SecureMessage*) pHeader;
    if (psmsg->version[0] != 1)
    {
        error("%s: Unknown version number.", __func__);
        return 2;
    };
    ...
    {
        error("%s: Address is not valid.", __func__);
        return 3;
    };
    if (!coinAddrDest.GetKeyID(ckidDest))
    {
    ...
    {
        std::string strPubKey, strPrivKey;
        if (!GetChannelKeys(address, strPubKey, strPrivKey))

        {
            error("%s: Could not get private key for addressDest.", __func__);
            return 3;
        }
        else
        {
    ...
    if (!cpkR.IsValid())
    {
        error("%s: Could not get pubkey for key R.", __func__);
        return 1;
    };
    ...
    {
        error("%s: Could not set pubkey for key R: %s.", __func__, HexStr(cpkR).c_str());
        return 1;
    };
    CECKey ecKeyDest;
...
//    // -- Do an EC point multiply with private key k and public key R. This gives you public key P.
    ...
    if (lenPdec != 32)
    {
        error("%s: ECDH_compute_key failed, lenPdec: %d.", __func__, lenPdec);
        return 1;
    };
...
//    // -- Message authentication code, (hash of timestamp + destination + payload)
    ...
    {
        error("%s: Decrypt failed.", __func__);
        return 1;
    };
    ...
    try {
        ...
        error("%s: msg.vchMessage.resize %u threw: %s.", __func__, lenPlain + 1, e.what());
        return 8;
    };
    ...
    if (lenPlain > 128)
    {
        ...
        if (LZ4_decompress_safe((char*) pMsgData, (char*) &msg.vchMessage[0], lenData, lenPlain) != (int) lenPlain)
        {
            ...
            error("%s: Could not decompress message data.", __func__);
            return 1;
        };
    } else
    {
    ...
    } else
    {
        ...
        {
            error("%s: From Address is invalid.", __func__);
            return 1;
        };
        ...
        {
            error("%s: Signature validation failed.", __func__);
            return 1;
        };
...
//int SecureMsgDecryptChannel(bool fTestOnly, std::string &address, std::string &pubkey, std::string &privkey, uint8_t *pHeader, uint8_t *pPayload, uint32_t nPayload, MessageData &msg)
//{
//    /* Decrypt secure message for channel
...
//    if (!fGood)
//    {
//        error("%s: Could not get private key for channel addressDest.Invalid private key encoding.", __func__);
//        return 3;
//    };

//    CKey keyDest = vchSecret.GetKey();
//    if (!keyDest.IsValid())
//    {
//        error("%s: Private key outside allowed range.", __func__);
//        return 3;
//    };
...
//    CPubKey cpkR(psmsg->cpkR, psmsg->cpkR+33);
//    //CECKey cpkR;
//    //cpkR.SetPubKey((unsigned char*)pubkey.c_str(),pubkey.size());//test TODO
//    if (!cpkR.IsValid())
//    {
//        error("%s: Could not get pubkey for key R.", __func__);
//        return 1;
...
//    };
//    CECKey ecKeyDest;
//    //keyDest.Set((unsigned char*)key.c_str(),(unsigned char*)key.end(),false);//TODO Check!!
//    //ecKeyDest.SetSecretBytes((unsigned char*)privkey.c_str());??????????
//    ecKeyDest.SetSecretBytes(keyDest.begin());
...
//void ChannelSubscribtion(std::string strAddress, std::string strPubKey,
...
//    smsgChannels.push_back(SecMsgChannel(std::string(strAddress), std::string(strPubKey), std::string(strPrivateKey), true, true));
